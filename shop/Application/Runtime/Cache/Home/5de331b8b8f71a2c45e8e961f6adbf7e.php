 ...
 Vendor('XunSearch.lib.XS'); $xs= new \XS('goods'); $search = $xs->search; $hostwords = $search->getHotQuery(); foreach ($hostwords as $k=>$v) { ?>
                        ...
                        <div class="footprint-box">
                            <div class="footprint-item">
                                ...
                                <div class="f-goods-list clearfix">
                                    ...
                                    <!-- 定位足迹 -->
                                    <?php if(empty($history)): ?><div style="height:100px;">
                                            <span style="font-size:16px;line-height:100px;color:red;">暂无浏览商品，快去逛逛吧~</span>   
                                        </div>
                                    <?php else: ?>
                                        <?php if(is_array($history)): foreach($history as $k=>$v): ?><div class="item">
                                                <a k="<?php echo ($k); ?>" class="g-del jin-del"><i class="iconfont icon-shanchu"></i></a>
                                                <a href="<?php echo U('Goods/index', ['id'=>$v['id']]);?>"><img class="figure" src="/Uploads/<?php echo ($v['pic1']); ?>" alt="" /></a>
                                                <div class="cont">
                                                    <div class="name"><a href=""><?php echo ($v['gname']); ?></a></div>
                                                    <div class="price">
                                                        <span class="n">￥<?php echo ($v['price']*($v['discount']/100)); ?></span><span class="s"><?php echo ($v['price']); ?></span>
                                                    </div>
                                                    <span style="color: gray;position: relative;left:119px;top:12px;"><?php echo ($v['save_time']); ?></span>
                                                </div>
    ...
    <script>
        $('.jin-del').click(function() {
            ...
            $.ajax({
                url: '/Home/History/ajaxDelHistory',
                type: 'get',
                data: {k: k},
                async: true,
